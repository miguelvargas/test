...
public class SourceFileGenerator {
	 ...
	 * @throws SIException Can't generate files
	 */
...
//		}
	...
	private void readTableInterface(ClassWriter cw, Annotation a, TableDefinition td)
	{
		...
		if ( td.name.length()==0)
			td.name=generateTableNameFromInterface(td.interfaceName);
		td.implementingClass=a.getElementValueAsString(cw, "ImplementingClassName");
		if ( td.implementingClass.length()==0)
			td.implementingClass= td.interfaceName + "Dao";
		
		td.makeAbstract=getElementValueBoolean( "ImplementingIsAbstract", cw, a, false);
		td.makePublic=getElementValueBoolean( "ImplementingIsPublic", cw, a, true);
	}
	
	private String generateTableNameFromInterface(String interfaceName) {
		String base = interfaceName;
		
		// Eliminate leading 'I' in names like IFoo
		char ch0 = interfaceName.charAt(0);
		char ch1 = interfaceName.charAt(1);
		if (ch0 == 'I' && ch1 >= 'A' && ch1 <= 'Z') { 
			base = interfaceName.substring(1);
		}
